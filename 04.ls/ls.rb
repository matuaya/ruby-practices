...
class LsOptiontA < LsCommand
  def modified_contents(dir)
    contents = Dir.entries(dir).sort
...
def display_with_options
  option = {}
  opt = OptionParser.new
  opt.on('-a') { |v| option[:a] = v }
  opt.parse!(ARGV)
  dir = ARGV[0] || '.'
  if option[:a] && ARGV.empty?
    LsOptiontA.new.display_contents('.')
    return
  elsif option[:a]
    LsOptiontA.new.display_contents(dir)
    return
  end
  LsCommand.new.display_contents(dir)
end
